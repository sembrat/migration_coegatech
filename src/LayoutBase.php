...
namespace Drupal\migration_coegatech;
 ...
 *
 * @package Drupal\migration_coegatech
 */
class LayoutBase extends ProcessPluginBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @param \Drupal\migration_coegatech\LayoutMigrationItem $item
   ...
   * @throws \Drupal\migration_coegatech\LayoutMigrationMissingBlockException
   */
  public function createComponent(LayoutMigrationItem $item, $region = 'content') {
    ...
    \Drupal::logger('migrate_news')->notice('adding block ' . $block_type . ' (' . $block_id  . ') in region ' . $region );
    if (!$block_type) {
    ...
    $block_revision_id = strval($this->blockContentStorage->getLatestRevisionId($block_id));
   ...
   * @param \Drupal\migration_coegatech\LayoutMigrationMissingBlockException $e
